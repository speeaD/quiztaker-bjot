...
const navigationItems = [
  ...
  { label: 'Study Hub', icon: BookOpen, href: '#' },
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col border-r border-[#dce5df] bg-white lg:flex">
      <div className="flex h-20 items-center gap-1 border-b border-[#e7eee9] px-2">
        ...
        <div><p className="text-[9px] font-bold tracking-[0.16em] text-[#a1610b]">STUDENT PORTAL</p></div>
      </div>

      <nav className="space-y-1 px-3 py-5" aria-label="Main navigation">
        {navigationItems.map(({ label, icon: Icon, href, active }) => (
          <Link key={label} href={href} className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-xs font-semibold transition ${active ? 'bg-[#0d3b2e] text-white shadow-sm' : 'text-[#526158] hover:bg-[#edf3ef] hover:text-[#0d3b2e]'}`}>
      ...
      <div className="m-3 mt-auto space-y-3">
        <div className="rounded-lg border border-[#dce5df] bg-[#edf3ef] p-3">
          <p className="text-[10px] font-bold uppercase tracking-wide text-[#526158]">Target readiness</p>
          <div className="mt-1 flex items-end justify-between"><strong className="text-base">{readiness}%</strong><span className="text-[10px] font-semibold text-[#286448]">Ready</span></div>
        ...
        <button type="button" onClick={() => void handleLogout()} disabled={isLoggingOut} className="flex w-full items-center justify-center gap-2 rounded-md border border-[#c9d8cf] px-3 py-2.5 text-xs font-bold text-[#15513e] transition hover:bg-[#edf3ef] disabled:cursor-not-allowed disabled:opacity-60">
          {isLoggingOut ? <Loader2 size={15} className="animate-spin" /> : <LogOut size={15} />}
          {isLoggingOut ? 'Logging out…' : 'Log out'}
        </button>
        {logoutError && <p role="alert" className="text-center text-[10px] font-medium text-[#b85b26]">{logoutError}</p>}
      </div>
    </aside>
  );
}
